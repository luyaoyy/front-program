              ...
              <template v-for="(item,index) of allRankMovieList">
                <li :key="index" v-if="index==0" @click="handleMovieDetail(item.movie)">
                  ...
                  <div style="display: inline-block">
                    <span style="display: inline-block;font-size: 20px;margin: 8px;font-style: italic;font-weight: 600">{{ item.movie.mname }}</span>
                    ...
                    <p><span class="suffix">实时票房:</span><span style="font-size: 18px;color:#ef4238 ">{{item.total}}</span></p>
                  </div>
                </li>
                <li :key="index" v-if="index==1||index==2" class="after" @click="handleMovieDetail(item.movie)">
                  <span style="float: left"><span style="font-size: 22px;color: #ef4238;font-weight: initial;font-style:italic;margin-right: 10px">{{ index + 1 }}</span>{{item.movie.mname}}</span>
                ...
                <li :key="index" v-if="index>=3" class="after" @click="handleMovieDetail(item.movie)">
                  <span class="name"><span style="font-size: 22px;color: #999;font-weight: initial;font-style:italic;margin-right: 10px">{{ index + 1 }}</span>{{item.movie.mname}}</span>
